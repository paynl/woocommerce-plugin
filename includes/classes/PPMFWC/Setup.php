...
class PPMFWC_Setup
{
    ...
    public static function ppmfwc_installBlog()
    {
        ...
        $check_tables = [
            $table_name_transactions => ($wpdb->get_var("SHOW TABLES LIKE '$table_name_transactions'") == $table_name_transactions),
            $table_name_options => ($wpdb->get_var("SHOW TABLES LIKE '$table_name_options'") == $table_name_options),
            $table_name_option_subs => ($wpdb->get_var("SHOW TABLES LIKE '$table_name_option_subs'") == $table_name_option_subs)
        ];

        if (empty($pay_db_version) || in_array(false, $check_tables)) {
            $sqlTransactions = '';
            if ($check_tables[$table_name_transactions] === false) {
            ...
            $sqlOptions = '';
            if ($check_tables[$table_name_options] === false) {
            ...
            $sqlOptionSub = '';
            if ($check_tables[$table_name_option_subs] === false) {
